...
class ApoliceResource extends Resource
{
                        ...
                        ->schema([
                            Infolists\Components\TextEntry::make('identificacao_segurado')
                                ...
                                ->state(fn (Model $record) => $record->segurado?->tipo === 'PF'
                                    ? $record->segurado?->seguradoPf?->nome
                                    : $record->segurado?->seguradoPj?->razao_social)
                                ->url(fn ($record) => $record->segurado_id ? SeguradoResource::getUrl('view', ['record' => $record->segurado_id]) : null)
                                ->color('warning'),
                            ...
                            Infolists\Components\TextEntry::make('filial.nome')
                                ->label('Filial')
                                ->url(fn ($record) => $record->filial_id ? FilialResource::getUrl('view', ['record' => $record->filial_id]) : null)
                                ->color('warning'),

                            Infolists\Components\TextEntry::make('cotacao_id')
                                ->label('Cotação')
                                ->formatStateUsing(fn ($state) => "Cotação #{$state}")
                                ->url(fn ($record) => $record->cotacao_id ? CotacaoResource::getUrl('view', ['record' => $record->cotacao_id]) : null)
                                ->color('warning'),

                            Infolists\Components\TextEntry::make('apolice_origem_id')
                                ->label('Apólice de Origem')
                                ->formatStateUsing(fn ($state) => "Apólice #{$state}")
                                ->url(fn ($record) => $record->apolice_origem_id ? ApoliceResource::getUrl('view', ['record' => $record->apolice_origem_id]) : null)
                                ->color('warning')
                        ...
                        ->schema([
                            ...
                            Infolists\Components\KeyValueEntry::make('dados_bem_assegurado')
                                ...
                                ->state(function ($record) {
                                    if (! $record->dados_bem_assegurado) {
                                        return [];
                                    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->recordAction(ViewAction::class) // Transforma o clique no gatilho da View
            ->filters([
                ...
                ActionGroup::make([
                    ...
                    Tables\Actions\Action::make('gerar_endosso')
                        ...
                        ->action(function (array $data, Model $record) {
                            ...
                            return redirect(ApoliceResource::getUrl('view', ['record' => $novaApolice->id]));
                        }),
                ]),
            ])
            ->bulkActions([
